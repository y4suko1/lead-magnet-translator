...
**If a transcript uses none of these, it says so plainly and stops rather than guessing at turn boundaries.** Its track record against real transcripts so far: an Otter export exposed a real bug (a label shape it matched wrong, silently misattributing a quote), caught by the required section 7b hand-check, then fixed here; a Fathom export used a shape it had never seen at all, correctly refused rather than guess, and that shape was added afterward; a real Zoom VTT export (a full masterclass transcript, not a fixture) ran clean, 32/32 lines and 11/11 claims confirmed, no count or attribution mismatches, and the notes-lines path was checked separately in the same session. All four shapes have now each been checked against real transcript content at least once.

**It cannot check everything section 7b covers.** Whether a quote actually supports the strength or characterisation a claim gives it ("the comparison worked," "the room reacted well") is a judgement call about meaning, not a structural fact, and this script can't settle it. A clean run here is not a substitute for the full section 7b recheck, every time, it's a second, independent cross-check on the part that genuinely is mechanical.

It does print `REVIEW` flags nominating a claim for closer attention during that manual step: an absolute word ("everyone", "always", "proves"), a characterisation word ("worked", "agreed", "reacted") sitting near a negation or contrast word in its own source quote, a source quote that's unusually short or isolated, or a claim whose `SOURCE:` line numbers sit far outside every other claim filed under the same `IDEA:` heading. **These are nominations, not verdicts** — a flagged claim might be perfectly honest, and an unflagged one could still be wrong, since none of this can actually read the transcript's meaning. They never affect the script's PASS/FAIL result. Treat them as "look here first," not "this is broken."

**Neither this script nor `check.py` checks that a claim is actually about the idea it's filed under.** Both work one claim at a time: they confirm a quote is real and, here, who said it and how many did — never whether a genuine, correctly-quoted claim has been filed under the right `IDEA:` heading rather than a different one it actually belongs to. The line-number outlier flag above is a cheap heuristic nomination for this specific gap, not a check for it: tested live (see `test-cases/idea-outlier-sources.txt`) against a real misfiled-claim case built from an actual transcript session, where it correctly caught a claim moved to another idea's heading; it can just as easily miss a real misfiling that happens to sit near other claims' line numbers, or flag a genuine callback to an earlier moment as a false positive. Confirm each claim actually belongs to its idea by hand during step 3, flagged or not.
