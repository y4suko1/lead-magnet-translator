...
It recognises three transcript label shapes: `SPEAKER: dialogue` on one line (including a Zoom VTT-style `Name (username): dialogue`, with the `(username)` dropped from the label it reports); the auto-transcript shape some tools (Otter.ai and similar) produce, a speaker label and timestamp alone on their own line with no colon (`JM  05:12`), dialogue starting the line after; and a Zoom VTT export's sequence-number-and-timestamp-range lines before a `Name (username):` line (both of those lines are ignored, not mistaken for dialogue or a label). **If a transcript uses none of these, it says so plainly and stops rather than guessing at turn boundaries** — this happened on the first real transcript it was run against, and it correctly refused rather than risk a wrong mapping; the person doing the check fell back to doing that part by hand, exactly as intended.
