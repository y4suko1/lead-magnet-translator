...
It also prints `REVIEW` flags nominating a claim for closer attention during step 3: an absolute word ("everyone", "always"), a characterization word sitting near a negation or contrast word in its own source quote, or an unusually short source quote. **A `REVIEW` flag is a nomination, not a verdict**, and it never affects the script's pass/fail exit code. Look at flagged claims first during step 3, but don't skip an unflagged claim either, since none of these checks can tell whether the transcript's meaning actually supports what a claim says.
